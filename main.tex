...
\JHPSissue{1}   % keep 0 until the issue is assigned
...
%The quantum intervals are allowed to be non-uniform, i.e., to be of different size.
%Theoretically, by shrinking or extending some intervals we can investigate particular performance ranges in more detail (not covered in this work).
...
%Secondly, the mean values are quantized into one NonIO level and 16 I/O levels according to \Cref{tab:quant_intervals}.
...
%    \end{tabular}
%  \end{subtable}
%\caption{Q-codes with non-uniform quantization intervals}
...
We don’t mention them in the paper.
...
This is a variant of B\_ALL where subsequent zero-sequences are reduced to a single zero segment before \Cref{eq:sim:bin_aggzeros} is applied.
...
First, we determine and trim the I/O intensive metrics and use the \Cref{eq:slicesim} to compute individual metrics similarities.
...
In \Cref{lst:pm_quant:phase_matching_example} we slide the shorter phase over the longer and compute similarity to the corresponding part such that we can find the best match.
...
They are listed in \Cref{lst:pm_quant:phase_matching_example:matches}.
...
The calculation is done in \Cref{lst:pm_quant:phase_matching_example:similarity}.
...
\begin{table}[!bt]
 ...
 \begin{subtable}{\textwidth}
   \centering
     \begin{tabular}{lln{1}{\rd}l}
       \rot{Job-IO-Utilization} & \rot{Job-IO-Problem-Time} & \rot{Job-IO-Balance} & \rot{B-coding}                       \\
			 \midrule
       4                        & 1                         & 0.4375000            & [118]                                     \\
 ...
 \begin{subtable}{\textwidth}
   \centering
     \begin{tabular}{n{1}{\rd}n{1}{\rd}n{1}{\rd}n{1}{\rd}n{1}{\rd}n{1}{\rd}n{1}{\rd}n{1}{\rd}l}
       \rot{1\_md\_file\_delete} & \rot{1\_md\_mod} & \rot{1\_md\_other} & \rot{1\_md\_read} & \rot{1\_read\_bytes} & \rot{1\_read\_calls} & \rot{1\_write\_bytes} & \rot{1\_write\_calls} & \rot{B-coding} \\
       \midrule
 ...
 \end{subtable}
 \caption{Jobs found in the same cluster with their profiles coding ($\epsilon = 0.03$).
Columns containing zeros only are omitted.}
...
\begin{figure}
  ...
  \includegraphics[width=4.61in,height=3.68in]{runtimes.png}
...
\begin{figure}
  \centering
  %\includegraphics[width=4.61in,height=2.76in]{image19_2.png}
  \includegraphics[width=\textwidth]{sim_exploration.png}
...
\begin{figure}
  \centering
  \includegraphics[width=4.61in,height=1.85in]{cum_num.png}
...
\begin{figure}
  \centering
   %\includegraphics[width=4.61in,height=6.44in]{image11_2.png}
  \includegraphics[width=\textwidth]{relevant_jobs_overview.png}
...
\FloatBarrier
\paragraph{B\_ALL}
For the B-algorithms, we show the statistics of the largest clusters as well in \Cref{tab:bin:largest_clusters} as there is an important aspect.
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=4.61in]{bin_all_top10.png}
...
The first clusters are identical to B\_ALL, e.g., as shown in \Cref{tab:bin:largest_clusters}.
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=4.61in,height=1.38in]{bin_aggzeros_top10.png}
...
\begin{figure}
	...
	\begin{subtable}{\textwidth}
      ...
      \end{tabular}
		\end{tiny}
		\caption{Centroid and Top 5 job phenotypes.
The metrics that have no I/O activity are not included in the table.}
		\label{cluster:hex_lev:top_jobs}
	\end{subtable}
	\medskip
	\begin{subfigure}{\textwidth}
		\centering
    \includegraphics[width=4.61in,height=1.39in]{hex_lev_top10.png}
		\caption{Length distribution in the cluster.}
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=4.61in,height=1.39in]{hex_native_top10.png}
		\caption{Length distribution in the cluster.}
...
For example, for Q\_PHASES, the job phenotypes in \Cref{cluster:pm_quant:top_jobs} in the row one and two are 100$\%$ similar, despite of different lengths.
...
\FloatBarrier
\paragraph{B\_ALL}
Information related to the cluster is in \Cref{cluster:use_case:bin_all}.
...
\FloatBarrier
\paragraph{B\_AGGZEROS}
Information related to the cluster of the job is in \Cref{cluster:use_case:bin_aggzeros}.
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=4.61in,height=1.39in]{bin_aggzeros_use_case.png}
		\caption{Length distribution in the cluster.}
...
Notice that there is a discrepancy between the job visualization in \Cref{fig:use_case} and the quantum codings.
While in \Cref{cluster:use_case:hex_lev} we can see a less intensive phase in the beginning and a high intensive phase afterwards, the quantum coding contains a more or less single high intensive I/O phase.
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=4.61in]{hex_lev_use_case.png}
		\caption{Length distribution in the cluster.}
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=4.61in]{hex_native_use_case.png}
		\caption{Length distribution in the cluster.}
...
\begin{figure}
    ...
    \includegraphics[width=4.61in,height=1.39in]{hex_phases_use_case.png}
		\caption{Length distribution in the cluster.}
...
%We explored binary, quantized, and the native (floating-point, e.g., mean) values.
...
Even if a better solution would be found, there is no guarantee that the generated model could be applied on another system.
...
\textit{We thank the reviewers Jean-Thomas Acquaviva, Suren Byna, Adrian Jackson, and George Markomanolis for their constructive feedback.}
...
The paper address a key topic in I/O analysis.
As instrumentation and profiling are starting to get in place in  data centers the amount of information generated in unmanageable at the human scale and methods need to be developed to summarize the I/O profile ad provide insight.
Taking this problem at its inception the article proposes, discuss and compares several ways to describe an job in respect of its I/O characteristics.
Not only the metrics are discussed but also the level of accuracy for the measurements.
Thus the authors explore the trade-off between accurate description and the need to accept loss of information to build clusters.
The quality of the clustering is analyzed and overall the authors are able to summarize 100.000 jobs in a limited amount of clusters.
Some qualitative analysis is performed on the clustering.
Overall the paper address an important problem, with broad implications for I/O architectures, job scheduling and work-flow organization in a data center.
The paper is well written and clear.
The data set is large enough to back-up the relevance of the method.
It remains that data are all coming from a single site (DKRZ) which is OK to assess the method but somehow limit the scope of the findings.
...
\paragraph{Overall summary and proposal for acceptance}
This paper describes an exploration to cluster one million jobs that ran on the Mistral system at DKRZ.
The authors used 10,000 jobs for training and then used the 1 million jobs for clustering.
It was found that general purpose clustering algorithms were not creating meaningful clusters, hence, the authors explored quantitative and qualitative analysis of clusters using knowledge of I/O patterns.


What does the “score” mean in Figure 2.2? Please define it as it is not obvious based on the figure’s caption.


In general, the motivation of this paper is interesting.
One thing I wasn’t clear at the end, may be due to my lack of expertise in clustering, which clustering method should a data center use.
Understandably, clustering depends heavily on similarity, but selection of a similarity could be a daunting task for users.
...
\paragraph{Overall summary and proposal for acceptance}
During the profiling of the I/O on a supercomputer to analyze data, a scientist has to handle a problem, there are a lot of data from various jobs.
New methodologies need to be developed for more efficient analysis.
The authors propose an approach that is applied on Mistral system at DKRZ to map the I/O on some specific metrics where they can apply clustering techniques to avoid having a million different profiling data to analyze.
Moreover, the authors did quantitative and qualitative analysis regarding their approach.
They evaluated many algorithms and the coding technique helped them to use the time series data to study the similarity.
The outcome was to have a small number of clusters that could analyze.
...
The paper is lengthy but needs to provide a lot of details for the reader to understand better.
It would be great if more sites were included in these experiments but this could be future work.
\paragraph{Scope}   % in regards to the journal, i.e., does the topic fit?
Yes.
\paragraph{Significance}   % of the research, minor vs. major
Above average
...
There are some places where the language or the discussion in the paper needs to be addressed, and this has been added in comments in the paper.
Of most importance are the figures \Cref{cluster:use_case:bin_aggzeros} and \Cref{cluster:use_case:bin_all} where I think the labels are the wrong way around at the moment.
There are also some issues around how levenshtein distance has been used in the research that could be improved or at least clarified, because it is not currently clear how text manipulations are being undertaken, i.e.
in the example [-:6:-:-] -> [-:388:-:-] is described as requiring one change, but for classical Levenshtein distance that should be a minimum of 3 changes.
...
\paragraph{Scope}   % in regards to the journal, i.e., does the topic fit?
Yes, the investigation of I/O monitoring metrics and analysis is a good fit to the journal

\paragraph{Significance}   % of the research, minor vs. major
The work has good significance in the topic of I/O metrics and analysis

\paragraph{Readability}   % English and structure appropriate?
In general the readability is good, although there are a number of places where the language could be improved or where text is not completely understandable.
Comments have been added in the text outlining these issues.
...
% Reviewers, please duplicate this section:
